...
if ( $engine eq "SGE" ) {
	$LSF         = "qsub";
	$LSF_OPTIONS = "-b y -j y -V -P 0380 -l medium";
} elsif ( $engine eq "LSF" ) {
	$LSF         = 'bsub';
...
while ( defined( $filename = readdir(DIR) ) ) {
	...
	if ( $filename !~ /$ext$/ ) {
		...
		#		print STDERR"$filename is not $ext file skipping\n";
		next;
	}
...
sub launch_job {
	...
	if ( stat $logfile ) {
		print STDERR "Removing $logfile\n";
		unlink($logfile);
	}

	if ( stat $outfile ) {
		print STDERR "Removing $outfile\n";
		unlink($outfile);
	}
