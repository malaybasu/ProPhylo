# $Id: SeqDB.pm 698 2013-04-15 16:24:36Z malay $
...
use SeqToolBox::Seq;
use IO::File;
use SeqToolBox::SeqDB::FASTAIndex;
use Carp;
use strict;
...
sub create_index {
	...
	unless ($index_file) {
		if (exists $self->{file}) {
			$index_file = $self->{file} . '.idx';
		}else {
			croak "Looks like you're using a file handle. Please give a index file name\n";
		}
	}
#	unless ($index_file && exists $self->{file}) {
#		
#		print STDERR "Index not given\n";
#		
...
#	}
	...
	if ($index_file) {
		...
		#		require DB_File || die "Can't load DB_File module\n";

		#		tie( %index, 'SDBM_File', $index_file, O_RDWR | O_CREAT, 0666 )
		#			|| die "Can't create tied hash\n";
		#		tie( %index, 'DB_File', $index_file, O_RDWR | O_CREAT, 0666 )
		#			|| die "Can't create tied hash\n";
		croak "$index_file already exists" if ( -s $index_file );
		$self->{seq_index}
			= SeqToolBox::SeqDB::FASTAIndex->new( -file => $index_file );
		$self->{seq_index}->create();
	}
...
#	}
	...
	while ( my $line = <$fh> ) {
		...
		if ( $line =~ /^>(\S+)/ ) {
			...
			#				print STDERR "Position: ", tell($fh), "\n";
			my $pos = ( tell($fh) - $filepos );
			#print STDERR $id, "\t", $pos, "\n";
			$self->{seq_index}->insert( -id => $id, -pos => $pos );
	...
	$self->{seq_index}->create_index();
	$self->{seq_index}->commit();
	$fh->setpos($current_pos);
...
sub get_seq_by_id {
	...
	# then check whether an index file has been supplied
		...
		#		require DB_File || die "Can't load DB_File\n";
		#		tie( %index, 'DB_File', $self->{file_index}, O_RDONLY, 0666 )
		#			|| die "Can't read index file\n";
		$seq_index = SeqToolBox::SeqDB::FASTAIndex->new(
												 -file => $self->{file_index} );
	}

	# See whether we can read the index file from disk
	elsif ( exists $self->{file} && -s $index_file_name ) {

		#		print STDERR "Id $id called\n";
		#		print STDERR "Index file name: $index_file_name\n";
		#		require DB_File
		#			|| die "Can't load DB_File\n";
		#		tie( %index, 'DB_File', $index_file_name, O_RDONLY, 0666 )
		#			|| die "Can't read index file\n";
		$seq_index
			= SeqToolBox::SeqDB::FASTAIndex->new( -file => $index_file_name );
	...
	if ($seq_index) {
		#print STDERR "Index exists\n";
		my $pos = $seq_index->get_pos($id);
		if ( defined ($pos)) {
			#print STDERR "Id $id pos: $pos\n";
			#		print STDERR $index{$id}, "\n";
			$fh->seek( $pos, 0 ) || die "Can't set to required position";
			$self->_parse_file();
